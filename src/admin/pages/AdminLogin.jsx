...
export default function AdminLogin() {
  ...
  const onSubmit = async (values) => {
    try {
      ...
      storage.setItem('p4_admin_token', data.token);
      localStorage.removeItem(values.remember ? 'unused' : 'p4_admin_token');
      sessionStorage.removeItem(values.remember ? 'p4_admin_token' : 'unused');
  ...
  return (
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#111111] p-4 text-white">
      ...
      <motion.form
        ...
        className="relative z-10 w-full max-w-md rounded-2xl border border-white/10 bg-[#1A1A1A]/90 p-8 shadow-[0_24px_90px_rgba(0,0,0,.45)] backdrop-blur-xl"
        initial={{ opacity: 0, y: 24 }}
        animate={{ opacity: 1, y: 0 }}
      >
        <p className="text-xs font-extrabold uppercase tracking-[0.24em] text-gold">Admin Login</p>
        <h1 className="mt-3 font-display text-4xl font-bold">P4 Properties</h1>
        ...
        <div className="mt-8 grid gap-4">
          ...
          <button disabled={isSubmitting} className="rounded-xl bg-gold px-6 py-4 text-sm font-extrabold uppercase tracking-[0.16em] text-night hover:bg-white disabled:opacity-60">
